...
def init():
    ...
    with sqlite3.connect(DATABASE_PATH) as c:
        ...
        event_id TEXT PRIMARY KEY,source TEXT,title TEXT,url TEXT,published_at TEXT,
        impact TEXT,direction TEXT,first_seen_at TEXT DEFAULT CURRENT_TIMESTAMP)''')
        c.execute('''CREATE TABLE IF NOT EXISTS news_alert_deliveries(
        id INTEGER PRIMARY KEY,event_id TEXT NOT NULL,chat_id INTEGER NOT NULL,payload TEXT NOT NULL,
        created_at TEXT DEFAULT CURRENT_TIMESTAMP,delivered_at TEXT,attempts INTEGER NOT NULL DEFAULT 0,
        last_error TEXT,UNIQUE(event_id,chat_id),
        FOREIGN KEY(event_id) REFERENCES news_events(event_id))''')
        c.execute('''CREATE INDEX IF NOT EXISTS idx_news_alert_deliveries_pending
            ON news_alert_deliveries(delivered_at,created_at)''')
...
def register_breaking_news(events):
    """Persist events atomically; suppress historical alerts on the first poll."""
    with sqlite3.connect(DATABASE_PATH) as c:
        initialized=c.execute(
            "SELECT 1 FROM runtime_state WHERE key='news_monitor_initialized'").fetchone()
        fresh=[]
        for event in events:
            cursor=c.execute('''INSERT OR IGNORE INTO news_events(
                event_id,source,title,url,published_at,impact,direction)
                VALUES(?,?,?,?,?,?,?)''',(
                    str(event["id"]),str(event.get("source","")),str(event.get("title","")),
                    str(event.get("url","")),event.get("published_at"),
                    str(event.get("impact","HIGH")),str(event.get("direction","NEUTRAL"))))
            if initialized and cursor.rowcount:
                fresh.append(event)
        if not initialized:
            c.execute('''INSERT INTO runtime_state(key,value,updated_at)
                VALUES('news_monitor_initialized','1',CURRENT_TIMESTAMP)
                ON CONFLICT(key) DO UPDATE SET value='1',updated_at=CURRENT_TIMESTAMP''')
        c.execute('''DELETE FROM news_alert_deliveries
            WHERE created_at<datetime('now','-30 days')''')
        c.execute('''DELETE FROM news_events WHERE first_seen_at<datetime('now','-30 days')
            AND NOT EXISTS(SELECT 1 FROM news_alert_deliveries d
                           WHERE d.event_id=news_events.event_id AND d.delivered_at IS NULL)''')
    return fresh

def enqueue_news_alert(event_id,chat_id,payload):
    with sqlite3.connect(DATABASE_PATH) as c:
        c.execute('''INSERT OR IGNORE INTO news_alert_deliveries(event_id,chat_id,payload)
            VALUES(?,?,?)''',(str(event_id),int(chat_id),str(payload)))

def pending_news_alerts(limit=100):
    with sqlite3.connect(DATABASE_PATH) as c:
        return c.execute('''SELECT d.id,d.event_id,d.chat_id,d.payload,d.attempts
            FROM news_alert_deliveries d
            JOIN subscribers sub ON sub.chat_id=d.chat_id AND sub.enabled=1
            WHERE d.delivered_at IS NULL AND d.created_at>=datetime('now','-24 hours')
            ORDER BY d.id LIMIT ?''',(int(limit),)).fetchall()

def mark_news_alert_sent(delivery_id):
    with sqlite3.connect(DATABASE_PATH) as c:
        c.execute('''UPDATE news_alert_deliveries SET delivered_at=CURRENT_TIMESTAMP,
            attempts=attempts+1,last_error=NULL WHERE id=? AND delivered_at IS NULL''',
            (int(delivery_id),))

def mark_news_alert_failed(delivery_id,error):
    with sqlite3.connect(DATABASE_PATH) as c:
        c.execute('''UPDATE news_alert_deliveries SET attempts=attempts+1,last_error=?
            WHERE id=? AND delivered_at IS NULL''',(str(error)[:500],int(delivery_id)))

def news_alert_stats():
    with sqlite3.connect(DATABASE_PATH) as c:
        pending=c.execute('''SELECT COUNT(*) FROM news_alert_deliveries
            WHERE delivered_at IS NULL AND created_at>=datetime('now','-24 hours')''').fetchone()[0]
        delivered=c.execute('''SELECT COUNT(*) FROM news_alert_deliveries
            WHERE delivered_at>=datetime('now','-24 hours')''').fetchone()[0]
    return {"pending":pending,"delivered_24h":delivered}
