"""Korkovts Signal AI V11.12.2 · AUTO PULSE."""
...
# V11.12.2 AUTO contract: full Futures scan/status every 10 minutes.
...
async def _send_auto_heartbeat(bot,chats,diagnostics,*,fresh_setups=0,triggered=0,scan_error=""):
    ...
    for chat_id in list(chats or []):
        try:
            ...
            core.log.exception("V11.12.2 AUTO heartbeat delivery failed chat=%s",chat_id)
    return delivered
...
async def run_automatic_scan_v11122(context,scanner_fn,label):
    ...
    try:
        ...
        while core._scan_lock.locked() and waited<90:
            ...
            waited+=5
        if core._scan_lock.locked():
            core.log.warning("V11.12.2 automatic %s skipped after waiting %ss",label,waited)
            if heartbeat:
        ...
        core.log.info(
            "V11.12.2 automatic %s setups=%s armed=%s cancelled=%s entry_now=%s waited=%ss",
        ...
        core.log.exception("V11.12.2 automatic %s scan failed",label)
...
async def _fast_analyze_main_symbol(symbol):
    """Run one symbol through the same full Production stack used by the broad scan."""
    if not await _health_gate("main"):
        return None
    token=begin_news_context()
    try:
        lower,base,higher,derivatives,state,news=await asyncio.gather(
            core.get_klines(symbol,"15m",320),
            core.get_klines(symbol,"1h",400),
            core.get_klines(symbol,"4h",400),
            core.get_derivatives_snapshot(symbol),
            core.market_state(),
        ...
        derivatives.update(core.liquidation_snapshot(symbol,oi_notional))
        analysis_state,_=core.market_analysis_state(state)
        threshold=float(thresholds_v112(state)["main"])
        result=core.analyze(
            symbol,"1H",base,higher,threshold,lower,analysis_state["bias"],
            derivatives,core.for_symbol(news,symbol),analysis_state
        )
        if not result:
            return None
        penalty=float(db.calibration_penalty(symbol,result.side,"1H") or 0)
        if penalty>0:
            result=core.analyze(
                symbol,"1H",base,higher,min(95.0,threshold+penalty),lower,
                analysis_state["bias"],derivatives,core.for_symbol(news,symbol),analysis_state
            )
            if not result:
                return None
        ...
        prepared=await _prepare([result],"main")
    ...
    async with _fast_radar_lock:
        ...
        try:
            ...
            async def one(pick):
                try:
                    async with sem:
                        signal=await asyncio.wait_for(_fast_analyze_main_symbol(pick.symbol),timeout=75)
                    if not signal:
                    ...
                    core.log.warning(
                        "V11.12.2 Fast Radar failed %s: %s: %s",pick.symbol,type(exc).__name__,exc
                    )
            ...
            core.log.info(
                "V11.12.2 Fast Radar checked=%s armed=%s triggered=%s symbols=%s",
            ...
            core.log.exception("V11.12.2 Fast Radar cycle failed")
...
async def system_v112(update,context):
    ...
    try:
        ...
        text=(
            "🛡 <b>KORKOVTS V11.12.2 AUTO PULSE</b>\n"
            "━━━━━━━━━━━━━━━━━━\n"
...
async def post_init_v112(application):
    ...
    application.job_queue.run_repeating(
        fast_radar_job,interval=FAST_RADAR_INTERVAL_SEC,first=70,name="v11122-fast-radar"
    )
    application.job_queue.run_repeating(
