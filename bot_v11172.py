"""Korkovts Signal AI V11.17.2 · DEEP AUDIT HARDENING."""
...
async def _annotate_indicator_edge_many(rows):
    ...
    if not rows:
        return []
    # Bound REST load: only the best already-qualified candidates can reach this layer.
    ranked=sorted(rows,key=lambda s:(float(getattr(s,"professional_rank",0) or 0),float(getattr(s,"score",0) or 0)),reverse=True)[:10]
...
async def _prepare(results,kind):
    ...
    chosen=annotate_strong_signals(chosen)
        ...
        try:
            await asyncio.wait_for(asyncio.to_thread(
                record_v11170_challenger,fresh,final_gate.eligible,snapshot_decision.snapshot_fingerprint,{
                    ...
                    "snapshot":snapshot_decision.as_dict(),"adaptive":getattr(adaptive_decision,"label",None),
                }
            ),timeout=3.0)
        except Exception:
            core.log.exception("V11.17.1 challenger shadow journal failed")
        if not final_gate.eligible:
            reason=final_gate.blockers[0] if final_gate.blockers else "V11.17 Final Risk Gateway blocked ENTRY"
            cancel_entry_arm(arm_id,reason)
            return None
    ...
    if arms:
        ...
        for row in arms[:6]:
            lines.append(
                ...
                f"{escape(str(row.get('timeframe','?')))} · confirm <b>{int(row.get('confirm_streak',0) or 0)}/2</b>"
            )
    else:
...
async def callback_v112(update,context):
    ...
    if data=="v1117:risk":
        await query.answer()
        r=_last_final_risk
        text=("🧿 <b>FINAL RISK GATE · V11.17.2</b>\n━━━━━━━━━━━━━━━━━━\n"
...
async def system_v112(update,context):
    ...
    try:
        ...
        text=(
            "🛡 <b>KORKOVTS V11.17.2 DEEP AUDIT HARDENING</b>\n"
            "━━━━━━━━━━━━━━━━━━\n"
...
async def alerts_on_v11130(update,context):
    ...
    await update.effective_message.reply_text(
        ...
        "🚨 ENTRY NOW: проверка <b>каждые 30 секунд</b>, отправка сразу при 2/2\n"
