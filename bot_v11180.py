"""Korkovts Signal AI V11.18.0 · REGIME + PORTFOLIO DEFENSE."""
...
from v112_alpha import annotate as annotate_alpha, _recent_aggressor
...
from v11110_futures_orderbook import (
    monitor as futures_l2_monitor, stop as stop_futures_l2_monitor,
    set_symbol_provider as set_futures_l2_symbol_provider, health as futures_l2_health,
    snapshot as futures_l2_snapshot,
)
from v11110_tape import init as init_v11110_tape, status as v11110_tape_status
from v11122_auto import (
    AUTO_FULL_SCAN_MIN, FAST_RADAR_INTERVAL_SEC, FAST_RADAR_MAX_CANDIDATES,
    choose_radar_symbols, heartbeat_text,
)
from v11140_performance import report_text as performance_report_text, annotate_decision_margin
from v11150_strong import annotate_many as annotate_strong_signals, annotate as annotate_strong_signal, assess as assess_strong_signal
from v11151_indicators import annotate as annotate_indicator_edge
from v11160_adaptive import assess_signal as assess_adaptive_edge, report_text as adaptive_edge_report_text
from v11160_replay import init as init_v11160_replay, record as record_v11160_replay, recent_text as replay_recent_text
from v11170_snapshot import assess as assess_market_snapshot
from v11170_execution import assess as assess_execution_reality
from v11170_risk import assess as assess_final_risk
from v11170_challenger import init as init_v11170_challenger, record as record_v11170_challenger, report_text as challenger_report_text
from v11170_attribution import report_text as attribution_report_text
from v11170_validation import no_future_rows
from v11180_defense import assess as assess_v11180_defense, required_confirmations as defense_required_confirmations
from v11130_news_intel import (
    get_news_sentiment as enhanced_news_sentiment, opinion_text as news_opinion_text,
    alert_text as news_alert_text, for_symbol as enhanced_news_for_symbol,
    latest_breaking_symbols as news_breaking_symbols, EXTRA_FEEDS as GLOBAL_NEWS_FEEDS,
)
from v11110_lease import (
    acquire_with_wait as acquire_v11110_lease, heartbeat as heartbeat_v11110_lease,
    release as release_v11110_lease, status as v11110_lease_status,
)
atexit.register(release_v11110_lease)
...
# V11.13 AUTO contract: full Futures scan/status every 10 minutes.
# Fast Radar only discovers candidates; it never bypasses Production/ENTRY gates.
config.AUTO_SCAN_INTERVAL_MIN=AUTO_FULL_SCAN_MIN
core.AUTO_SCAN_INTERVAL_MIN=AUTO_FULL_SCAN_MIN
# Shock/news radar: poll once per minute. Headlines trigger rechecks, never direct ENTRY.
config.NEWS_POLL_INTERVAL_SEC=60
core.NEWS_POLL_INTERVAL_SEC=60
_fast_radar_lock=asyncio.Lock()
_fast_ticker_snapshot={}
_fast_radar_state={
    "status":"warming","checked":0,"armed":0,"triggered":0,"last_error":"",
    "last_run":0.0,"last_symbols":[],
}
_auto_trigger_events=[]
_last_prime_entry_at=0.0
_last_prime_entry_arm_id=None

class _EntryRevalidationWait(RuntimeError):
    """Transient market/data condition: keep the ARMED setup alive for recheck."""

PRIME_LABEL_LOCK_SEC=120

def _record_auto_trigger(row,signal,signal_id):
    source=str((row or {}).get("source") or "")
    if source.startswith("auto_"):
        _auto_trigger_events.append((time.time(),str(signal.symbol),str(signal.side),int(signal_id)))
        cutoff=time.time()-900
        _auto_trigger_events[:]=[x for x in _auto_trigger_events if x[0]>=cutoff]

def _recent_auto_triggers(seconds=600):
    cutoff=time.time()-max(1,int(seconds))
    _auto_trigger_events[:]=[x for x in _auto_trigger_events if x[0]>=time.time()-900]
    return [x for x in _auto_trigger_events if x[0]>=cutoff]


_indicator_edge_sem=asyncio.Semaphore(4)
_ui_background_tasks=set()
_last_final_risk={"status":"WARMING","symbol":"","score":0.0,"reason":"no final decision yet","at":0.0}

def _spawn_ui_task(coro,label):
    """Detach heavy button work after immediate callback ACK."""
    task=asyncio.create_task(coro,name=f"ui-{label}")
    _ui_background_tasks.add(task)
    def done(t):
        _ui_background_tasks.discard(t)
        try:
            exc=t.exception()
        except asyncio.CancelledError:
            return
        if exc is not None:
            core.log.error("Detached UI task %s failed: %r",label,exc,exc_info=exc)
    task.add_done_callback(done)
    return task

def _spawn_sync_text_reply(query,func,label):
    async def work():
        text=await asyncio.to_thread(func)
        return await query.message.reply_text(text,parse_mode=ParseMode.HTML,reply_markup=main_menu())
    return _spawn_ui_task(work(),label)

async def _annotate_indicator_edge_one(signal):
    """Fetch bounded closed-candle/aggTrade context and fail closed on missing data."""
    interval="15m" if str(getattr(signal,"timeframe","1H")).upper()=="15M" else "1h"
    try:
        async with _indicator_edge_sem:
            frame,agg=await asyncio.gather(
                asyncio.wait_for(core.get_klines(signal.symbol,interval,180),timeout=8),
                asyncio.wait_for(_recent_aggressor(signal.symbol),timeout=8),
            )
        validation=no_future_rows(frame,time.time())
        signal.feature_snapshot.setdefault("validation_v11170",{}).update(validation.as_dict()|{"schema":"11.17-validation-guards-v1"})
        if not validation.eligible:
            raise RuntimeError(f"indicator lookahead guard: {validation.reason}")
        imbalance,coverage=agg if isinstance(agg,tuple) and len(agg)>=2 else (None,0.0)
        # A tiny aggTrade sample is not treated as independent confirmation.
        if float(coverage or 0)<5.0:
            imbalance=None
        annotate_indicator_edge(signal,frame,imbalance)
    except Exception as exc:
        signal.indicator_edge_score=0.0
        signal.indicator_edge_support=0
        signal.indicator_edge_auto=False
        signal.indicator_edge_prime=False
        signal.feature_snapshot.setdefault("indicator_edge_v11151",{}).update({
            "schema":"11.15.1-indicator-edge-v1","available":False,
            "auto_eligible":False,"prime_eligible":False,
            "blockers":[f"indicator data unavailable: {type(exc).__name__}"],
            "negative_only":True,"professional_rank_changed":False,
        })
    return signal


async def _annotate_indicator_edge_many(rows):
    rows=list(rows or [])
    if not rows:
        return []
    # The scanner has already bounded the qualified pool to V11_CANDIDATE_POOL.
    # Do not silently drop candidates #11-20 here: earlier candidates may fail
    # Indicator/Adaptive/portfolio gates, so every bounded qualified candidate
    # must receive the same negative-only Indicator Edge check.
    ranked=sorted(rows,key=lambda s:(float(getattr(s,"professional_rank",0) or 0),float(getattr(s,"score",0) or 0)),reverse=True)[:V11_CANDIDATE_POOL]
    return list(await asyncio.gather(*(_annotate_indicator_edge_one(s) for s in ranked)))


def _indicator_gate(signal,prime=False):
    d=dict((getattr(signal,"feature_snapshot",{}) or {}).get("indicator_edge_v11151") or {})
    return bool(d.get("prime_eligible") if prime else d.get("auto_eligible"))

_card_v11100=card
def card(s,priority=False):
    text=_card_v11100(s,priority)
    try:
        margin=getattr(s,"decision_margin",None)
        label=str(getattr(s,"decision_margin_label","") or "")
        if priority and label:
            names={"CLEAR_PRIME":"🏆 CLEAR PRIME","CLOSE_RACE":"⚖️ CLOSE RACE","NEAR_TIE":"⚠️ NEAR TIE","PRIME":"🏆 PRIME"}
            shown=names.get(label,label)
            tail=(f" · gap <b>{float(margin):+.2f}</b>" if margin is not None else "")
            text+=f"\n\n🎯 <b>DECISION MARGIN</b> · <b>{shown}</b>{tail}"
        gate=(getattr(s,"feature_snapshot",{}) or {}).get("entry_gate_v11120") or (getattr(s,"feature_snapshot",{}) or {}).get("entry_gate_v11110") or {}
        if gate:
            ok=bool(gate.get("ok")); score=float(gate.get("stability_score",gate.get("score",0)) or 0)
            samples=int(gate.get("samples",0) or 0)
            flow_s=int(gate.get("flow_active_seconds",0) or 0)
            burst=float(gate.get("flow_max_bucket_share",0) or 0)
            text+=("\n\n🛡 <b>LIVE ENTRY GATE</b> · "
                   f"<b>{'CONFIRMED' if ok else 'WAIT'}</b> · L2 <b>{score:.0f}</b> · "
                   f"samples <b>{samples}</b> · flow <b>{flow_s}s</b> · "
                   f"burst <b>{burst:.0%}</b>")
        edge=dict((getattr(s,"feature_snapshot",{}) or {}).get("indicator_edge_v11151") or {})
        if edge:
            text+=("\n\n📐 <b>INDICATOR EDGE</b> · "
                   f"<b>{'PRIME' if edge.get('prime_eligible') else ('STRONG' if edge.get('auto_eligible') else 'BLOCK')}</b> · "
                   f"{float(edge.get('score',0) or 0):.0f}/100 · "
                   f"confirm <b>{int(edge.get('support',0) or 0)}/5</b> · "
                   f"RVOL <b>{float(edge.get('rvol',0) or 0):.2f}x</b>")
        final=dict((getattr(s,"feature_snapshot",{}) or {}).get("final_risk_gateway_v11170") or {})
        exr=dict((getattr(s,"feature_snapshot",{}) or {}).get("execution_reality_v11170") or {})
        if final:
            text+=("\n\n🧿 <b>FINAL RISK GATE</b> · "
                   f"<b>{escape(str(final.get('label','CHECK')))}</b> · {float(final.get('score',0) or 0):.0f}/100")
        if exr:
            i1=exr.get("impact_1000_bps")
            impact="n/a" if i1 is None else f"{float(i1):.1f}bps"
            text+=("\n⚙️ Execution reality · "
                   f"fresh <b>{float(exr.get('freshness',0) or 0):.0%}</b> · "
                   f"$1k <b>{impact}</b>")
    except Exception:
        pass
    return text

_system_extra_v11100=system_extra
def system_extra():
    base=_system_extra_v11100()
    try:
        l2=futures_l2_health(); tape=v11110_tape_status(); lease=v11110_lease_status(); live=live_health()
        routeline=(f"🛰 WS routes · PUBLIC <b>{'ON' if live.get('public_connected') else 'OFF'}</b> · "
                   f"MARKET <b>{'ON' if live.get('market_connected') else 'OFF'}</b> · "
                   f"self-heal <b>{int(live.get('route_stalls_public',0))+int(live.get('route_stalls_market',0))}</b>")
        l2line=(f"🧱 Futures L2 <b>{int(l2.get('healthy',0))}/{int(l2.get('total',0))}</b> healthy · "
                f"gaps <b>{int(l2.get('gaps',0))}</b> · resyncs <b>{int(l2.get('resyncs',0))}</b>")
        tapeline=(f"🎞 Tape <b>{int(tape.get('bundles',0))}</b> bundles · buffer <b>{int(tape.get('buffer_events',0))}</b> events")
        leaseline=(f"🔒 Singleton <b>{'HELD' if lease.get('ours') and lease.get('held') else 'CHECK'}</b>")
        radarline=(f"⚡ Auto Pulse <b>{str(_fast_radar_state.get('status','warming')).upper()}</b> · "
                   f"checked <b>{int(_fast_radar_state.get('checked',0))}</b> · "
                   f"armed <b>{int(_fast_radar_state.get('armed',0))}</b> · 60s")
        final=_last_final_risk
        finalline=(f"🧿 Final Risk <b>{escape(str(final.get('status','WARMING')))}</b> · "
                   f"{escape(str(final.get('symbol','—') or '—'))} · <b>{float(final.get('score',0) or 0):.0f}/100</b>")
        return base+"\n"+routeline+"\n"+l2line+"\n"+tapeline+"\n"+leaseline+"\n"+radarline+"\n"+finalline
    except Exception:
        return base+"\n🧱 Futures L2 <b>WARMING/CHECK</b>"
...
_original_news_triggered_scans=getattr(core,"_run_news_triggered_scans",None)
_news_rescan_task=None


_original_scanner_news=getattr(scanner,"get_news_sentiment",None)
_original_core_news=getattr(core,"get_news_sentiment",None)
_original_scanner_for_symbol=getattr(scanner,"for_symbol",None)
_original_core_for_symbol=getattr(core,"for_symbol",None)
...
def _adaptive_gate(signal):
    """Mature negative-only decay guard; small samples remain LEARNING."""
    d=assess_adaptive_edge(signal)
    return bool(d.eligible),d


async def _enhanced_news(fetcher,*args,**kwargs):
    async def wrapped(*inner_args,**inner_kwargs):
        return await enhanced_news_sentiment(fetcher,*inner_args,**inner_kwargs)
    return await safe_news_fetch(wrapped,*args,**kwargs)


async def _safe_scanner_news(*args,**kwargs):
    return await _enhanced_news(_original_scanner_news,*args,**kwargs)


async def _safe_core_news(*args,**kwargs):
    return await _enhanced_news(_original_core_news,*args,**kwargs)
...
if _original_scanner_for_symbol is not None:
    scanner.for_symbol=lambda snapshot,symbol: enhanced_news_for_symbol(_original_scanner_for_symbol,snapshot,symbol)
if _original_core_for_symbol is not None:
    core.for_symbol=lambda snapshot,symbol: enhanced_news_for_symbol(_original_core_for_symbol,snapshot,symbol)
...
def _analyze_with_freshness(*args,**kwargs):
    ...
    if result is not None and derivatives is not None:
        ...
        result.feature_snapshot.setdefault("data_freshness_v1142",{}).update({
            ...
            "adl_age_minutes":(float((derivatives or {}).get("adl_age_minutes"))
                               if (derivatives or {}).get("adl_age_minutes") is not None else 9999.0),
...
async def _prepare(results,kind):
    ...
    # V11.16.0 quality-first Indicator Edge Pack. This is a negative-only
    # final confirmation layer: AVWAP+POC, CVD+aggTrade pulse, RVOL, OI matrix
    # and liquidity-sweep safety. Correlated measurements are grouped and never
    # inflate professional_rank or the independent-evidence family count.
    indicator_checked=await _annotate_indicator_edge_many(protection_valid)
    indicator_rejected=[s for s in indicator_checked if not _indicator_gate(s)]
    protection_valid=[s for s in indicator_checked if _indicator_gate(s)]

    # V11.16 Adaptive Edge is mature and negative-only. Positive history never
    # promotes a candidate; sufficiently mature decaying cohorts are removed
    # from AUTO/ENTRY until forward performance recovers.
    adaptive_valid=[]
    adaptive_rejected=[]
    for signal in protection_valid:
        ok,_adaptive=_adaptive_gate(signal)
        if ok: adaptive_valid.append(signal)
        else: adaptive_rejected.append(signal)
    protection_valid=adaptive_valid

    # Strong Consensus is a real AUTO/ENTRY gate, so it must run BEFORE
    # portfolio truncation. Otherwise a non-Strong top-ranked candidate can take
    # one of the 3-4 portfolio slots, fail later in _arm_and_check(), and prevent
    # a slightly lower-ranked but genuinely Strong candidate from ever being seen.
    strong_checked=annotate_strong_signals(protection_valid)
    strong_valid=[]
    strong_rejected=[]
    for signal in strong_checked:
        strong=assess_strong_signal(signal)
        if strong.auto_eligible:
            strong_valid.append(signal)
        else:
            strong_rejected.append(signal)
    protection_valid=strong_valid
    ...
    chosen=annotate_decision_margin(chosen,protection_valid)
    ...
    record_many_v11100_blackbox(indicator_rejected,"INDICATOR_EDGE_REJECT",scan_id=scan_id,pipeline=kind)
    record_many_v11100_blackbox(adaptive_rejected,"ADAPTIVE_EDGE_REJECT",scan_id=scan_id,pipeline=kind)
    record_many_v11100_blackbox(strong_rejected,"STRONG_CONSENSUS_REJECT",scan_id=scan_id,pipeline=kind)
    ...
    for s in indicator_rejected:
        _store_counterfactual(s,"V11151_INDICATOR_EDGE_REJECT")
    ...
    try:
        ...
        d["evidence_rejected"]=len(evidence_rejected)
        d["meta_rejected"]=len(meta_rejected)
        d["protection_rejected"]=len(protection_rejected)
        d["indicator_rejected"]=len(indicator_rejected)
        d["adaptive_rejected"]=len(adaptive_rejected)
        d["strong_rejected"]=len(strong_rejected)
...
async def _health_gate(kind):
    ...
    if not clock.ok:
        try:
            ...
            d["status"]="blocked"; d["reason"]="BINANCE CLOCK PAUSE"; d["final"]=0
    ...
    if _last_health.hard_pause:
        try:
            ...
            d["status"]="blocked"; d["reason"]="PRODUCTION HEALTH PAUSE"; d["final"]=0
...
async def _run_scan_with_watchdog(kind,raw,timeout_sec):
    ...
    try:
        ...
        try:
            d=scanner._last_scan.setdefault(kind,{})
            d["status"]="error"
            d["reason"]=f"Production pipeline: {type(exc).__name__}: {exc}"
...
async def _refresh_candidate_v1142(row):
    ...
    if not await _health_gate(kind):
        raise _EntryRevalidationWait("production health/clock gate is temporarily unavailable")
    ...
    try:
        ...
        if not derivatives.get("deep_data"):
            raise _EntryRevalidationWait("fresh derivatives snapshot is incomplete")
...
async def _arm_and_check(signal,source):
    strong=assess_strong_signal(signal)
    annotate_strong_signal(signal)
    if not strong.auto_eligible or not _indicator_gate(signal) or not _adaptive_gate(signal)[0]:
        edge=dict((getattr(signal,"feature_snapshot",{}) or {}).get("indicator_edge_v11151") or {})
        adaptive_ok,adaptive=_adaptive_gate(signal)
        reason=(
            (edge.get("blockers") or [None])[0] if not _indicator_gate(signal) else None
        ) or (adaptive.reason if not adaptive_ok else None) or (strong.blockers[0] if strong.blockers else
                f"Strong Consensus {strong.score:.0f}/100 below AUTO/ENTRY floor")
        assessment=TriggerAssessment(
            state="CANCEL",score=float(strong.score),reason=reason,
            price=0.0,bid=0.0,ask=0.0,spread_bps=0.0,distance_r=0.0,
            one_min_ok=False,three_min_ok=False,flow_ok=False,flow_share=.5,
            flow_source="strong-consensus",candle_ok=False,volume_ratio=0.0,checked_at=time.time()
        )
        return _decorate_entry_state(signal,"CANCELLED",assessment,None,0),assessment,0
...
def _prime_label_candidate(arm_id,row):
    """At most one asynchronous ENTRY NOW may carry the №1 label in a short burst."""
    now=time.time()
    if now-float(_last_prime_entry_at or 0)<PRIME_LABEL_LOCK_SEC:
        return False
    active=active_entry_rows(30)
    if not active:
        return True
    best=max(active,key=lambda r:(float(r.get("source_pro") or 0),-float(r.get("armed_at") or 0)))
    return int(best.get("id") or 0)==int(arm_id)
...
async def _trigger_arm(context,arm_id,initial_assessment=None,force_chat_id=None):
    ...
    global _last_prime_entry_at,_last_prime_entry_arm_id
    async with _entry_now_lock:
        ...
        try:
            ...
            core.log.info(
                "V11.18 ENTRY WAIT %s %s %s: %s",
                row.get("symbol"),row.get("side"),row.get("timeframe"),exc,
            )
        ...
        strong=assess_strong_signal(fresh)
        annotate_strong_signal(fresh)
        if not strong.auto_eligible or not _indicator_gate(fresh) or not _adaptive_gate(fresh)[0]:
            edge=dict((getattr(fresh,"feature_snapshot",{}) or {}).get("indicator_edge_v11151") or {})
            adaptive_ok,adaptive=_adaptive_gate(fresh)
            reason=((edge.get("blockers") or [None])[0] if not _indicator_gate(fresh) else None) or (
                adaptive.reason if not adaptive_ok else None
            ) or (strong.blockers[0] if strong.blockers else
                f"Strong Consensus {strong.score:.0f}/100 below final ENTRY floor"
            )
            cancel_entry_arm(arm_id,reason)
            return None
        ...
        # V11.17 non-bypassable final gateway. Freeze one coherent local L2
        # snapshot, age the original READY trigger across full revalidation and
        # simulate executable depth before any Production row can be created.
        global _last_final_risk
        book=futures_l2_snapshot(fresh.symbol,3.0,100)
        snapshot_decision=assess_market_snapshot(fresh,book)
        trigger_ts=float(getattr(initial_assessment,"checked_at",0) or getattr(final_assessment,"checked_at",0) or time.time())
        execution_decision=assess_execution_reality(fresh,book,trigger_ts)
        adaptive_ok,adaptive_decision=_adaptive_gate(fresh)
        defense_decision=assess_v11180_defense(
            fresh,book,confirm_streak=int(row.get("confirm_streak") or 0),health=_last_health
        )
        final_gate=assess_final_risk(
            fresh,strong,_indicator_gate(fresh),adaptive_ok,
            snapshot_decision,execution_decision,
            safety_allow_live=bool(futures_safety_status().allow_live),
            defense=defense_decision,
        )
        _last_final_risk={
            "status":final_gate.label,"symbol":fresh.symbol,"score":final_gate.score,
            "reason":(final_gate.blockers[0] if final_gate.blockers else "pass"),"at":time.time(),
        }
        try:
            await asyncio.wait_for(asyncio.to_thread(
                record_v11170_challenger,fresh,final_gate.eligible,snapshot_decision.snapshot_fingerprint,{
                    "final_risk":final_gate.as_dict(),"execution":execution_decision.as_dict(),
                    "snapshot":snapshot_decision.as_dict(),"defense":defense_decision.as_dict(),"adaptive":getattr(adaptive_decision,"label",None),
                }
            ),timeout=3.0)
        except Exception:
            core.log.exception("V11.18 challenger shadow journal failed")
        if not final_gate.eligible:
            # Final snapshot/execution/defense blockers describe whether ENTRY is
            # safe *now*.  They must never destroy an otherwise valid ARMED setup:
            # L2, spread, adverse selection, health and the V11.18 3-check regime
            # persistence can recover on the next 30-second observation.  The arm
            # remains fail-closed and cannot reach save_pending() until every gate
            # passes in one fresh coherent snapshot.
            reason=final_gate.blockers[0] if final_gate.blockers else "Final Risk Gateway waiting"
            _last_final_risk["status"]="WAIT"
            _last_final_risk["reason"]=reason
            core.log.info(
                "V11.18 FINAL WAIT %s %s %s score=%.0f reason=%s",
                fresh.symbol,fresh.side,fresh.timeframe,float(final_gate.score),reason,
            )
        ...
        priority_label=bool(strong.prime_eligible and _indicator_gate(fresh,prime=True) and _prime_label_candidate(arm_id,row))
        signal_id=core.save_pending(fresh)
        record_v1180_decision("FUTURES",signal_id,fresh)
        try:
            await asyncio.to_thread(
                record_v11160_replay,signal_id,fresh,final_assessment,{
                    "arm_id":int(arm_id),"priority_label":bool(priority_label),
                    "source":str(row.get("source") or ""),
                }
            )
        except Exception:
            core.log.exception("V11.17.1 ENTRY replay capture failed signal=%s",signal_id)
        payload=core.fmt(fresh,priority_label)
        ...
        if delivered>0:
            ...
            if priority_label:
                _last_prime_entry_at=time.time()
                _last_prime_entry_arm_id=int(arm_id)
            _record_auto_trigger(row,fresh,signal_id)
            core.log.info(
                "V11.13 ENTRY NOW %s %s %s readiness=%.0f queued=%s delivered=%s",
...
async def signal_v1123(update,context):
    ...
    try:
        ...
        # V11.16.0: manual symbol analysis must pass the same Indicator Edge
        # layer as broad/AUTO scans; there is no manual bypass.
        await _annotate_indicator_edge_one(result)
        if not _indicator_gate(result):
            edge=dict((result.feature_snapshot or {}).get("indicator_edge_v11151") or {})
            reason=(edge.get("blockers") or ["Indicator Edge did not confirm the setup"])[0]
            return await msg.reply_text(
                f"⚪ <b>{symbol}</b>: setup rejected by Indicator Edge.\n"
                f"Причина: <b>{escape(str(reason))}</b>",
                parse_mode=ParseMode.HTML,reply_markup=main_menu(symbol)
            )
        ...
        if assessment.state=="READY":
            required_checks=defense_required_confirmations(getattr(result,"production_regime",""))
            await msg.reply_text(
                f"🟠 <b>{symbol} · ENTRY READY · CONFIRMATION 1/{required_checks}</b>\n"
                f"Первое micro-подтверждение есть. Для текущего режима нужны {required_checks} последовательные проверки. Жду следующую проверку…",
                parse_mode=ParseMode.HTML,reply_markup=main_menu(symbol)
            )
        ...
        return await msg.reply_text(
            ...
            "Когда 1m + 3m + taker-flow + spread подтвердятся устойчиво "
            "(минимум 2 раза, в защитных режимах 3), придёт отдельное <b>🚨 ENTRY NOW</b>.\n\n"
            +core.fmt(result,True),
            parse_mode=ParseMode.HTML,
            reply_markup=signal_actions(snapshot_id=snapshot_id)
        )
...
async def start_v112(update,context):
    ...
    text=(
        ...
        "🚨 <b>ENTRY NOW</b> — только подтверждённый вход 2/2; в защитных режимах 3/3</blockquote>\n"
        "\n🧠 <b>AI CORE</b>\n"
        "Block-bootstrap · Cohort Guard · Data Coherence · Black Box · Full Entry Replay\n"
        f"\n📡 Full AUTO <b>{core.AUTO_SCAN_INTERVAL_MIN}m</b> · Fast Radar <b>60s</b> · Entry monitor <b>30s</b> · Futures L2 <b>100ms</b>\n"
...
async def send_results_v112(bot,chat_id,results,automatic=False,short=False,diagnostics=None):
    ...
    await bot.send_message(
        chat_id,
        f"{'⚡ SHORT' if short else '🏆 MAIN'} <b>· V11.12 LIVE ENTRY</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
...
def _scan_pause_reason(diagnostics):
    d=dict(diagnostics or {})
    reason=str(d.get("reason") or "")
    upper=reason.upper()
    if any(token in upper for token in ("PAUSE","BLOCKED","CLOCK","HEALTH")):
        return reason or "production safety pause"
    if str(d.get("status") or "").lower() in {"error","blocked"}:
        return reason or f"scan status {d.get('status')}"
    return ""


async def _send_auto_heartbeat(bot,chats,diagnostics,*,fresh_setups=0,triggered=0,scan_error=""):
    """Mandatory 10-minute status message for Futures AUTO mode."""
    pause=_scan_pause_reason(diagnostics)
    effective_error=str(scan_error or pause or "")
    text=heartbeat_text(
        diagnostics,
        active_arms=len(active_entry_rows(30)),
        fresh_setups=int(fresh_setups),
        triggered=int(triggered),
        triggered_window=len(_recent_auto_triggers(600)),
        fast_radar_checked=int(_fast_radar_state.get("checked",0)),
        fast_radar_armed=int(_fast_radar_state.get("armed",0)),
        scan_error=effective_error,
    )
    delivered=0
    for chat_id in list(chats or []):
        try:
            await bot.send_message(
                int(chat_id),text,parse_mode=ParseMode.HTML,reply_markup=main_menu()
            )
            delivered+=1
        except Exception:
            core.log.exception("V11.13 AUTO heartbeat delivery failed chat=%s",chat_id)
    return delivered


async def run_automatic_scan_v11122(context,scanner_fn,label):
    """10-minute full scan + mandatory status; ENTRY NOW remains immediate."""
    chats=[]
    ...
    try:
        ...
        while core._scan_lock.locked() and waited<90:
            ...
            waited+=5
        if core._scan_lock.locked():
            core.log.warning("V11.13 automatic %s skipped after waiting %ss",label,waited)
            if heartbeat:
                await _send_auto_heartbeat(
                    context.bot,chats,scanner.scan_status().get("main",{}),
                    scan_error="предыдущий скан не освободил scan-lock за 90 секунд"
                )
            return
        ...
        triggered=0
        for result in fresh:
            result,assessment,streak=await _arm_and_check(result,f"auto_{label}")
            if assessment.state=="CANCEL":
                cancelled+=1
                continue
            armed+=1
        ...
        core.log.info(
            "V11.13 automatic %s setups=%s armed=%s cancelled=%s entry_now=%s waited=%ss",
            label,len(fresh),armed,cancelled,triggered,waited
        )
        # The offset 15M scan remains silent unless ENTRY NOW actually fires;
        # otherwise AUTO would spam a second no-signal message five minutes later.
        if heartbeat:
            await _send_auto_heartbeat(
                context.bot,chats,scanner.scan_status().get("main",{}),
        ...
        core.log.exception("V11.13 automatic %s scan failed",label)
...
core._run_automatic_scan=run_automatic_scan_v11122


async def _fast_analyze_symbol(symbol,timeframe):
    """Run one symbol/timeframe through the same full Production stack as broad scans."""
    ...
    try:
        ...
        threshold=float(thresholds_v112(state)[threshold_key])
        result=core.analyze(
            symbol,timeframe,base,higher,threshold,lower,analysis_state["bias"],
            derivatives,core.for_symbol(news,symbol),analysis_state
        )
        if not result:
            return None
        penalty=float(db.calibration_penalty(symbol,result.side,timeframe) or 0)
        ...
        if bool((news or {}).get("v114_news_degraded")):
            ...
            result.feature_snapshot.setdefault("news",{}).update({
                "degraded":True,"sources":0,"real_sources":0,
                "degraded_reason":str((news or {}).get("v114_news_reason") or "unavailable"),
            })
        prepared=await _prepare([result],kind)
...
async def _fast_analyze_main_symbol(symbol):
    return await _fast_analyze_symbol(symbol,"1H")


async def _fast_analyze_short_symbol(symbol):
    return await _fast_analyze_symbol(symbol,"15M")


async def fast_radar_job(context):
    """60-second focused discovery between full scans; never bypasses Production gates."""
    global _fast_ticker_snapshot,_fast_radar_state
    if _fast_radar_lock.locked() or core._scan_lock.locked():
        return
    if not core.subscribers():
        return
    async with _fast_radar_lock:
        started=time.time()
        try:
            tickers=await core.get_tickers()
            near=list(news_breaking_symbols())
            try:
                for row in (scanner.scan_status().get("main",{}).get("near_candidates") or []):
                    symbol=str((row or {}).get("symbol") or "").upper()
                    if symbol and symbol not in near:
                        near.append(symbol)
            except Exception:
                pass
            picks,new_snapshot=choose_radar_symbols(
                tickers,_fast_ticker_snapshot,near_symbols=near,
                active_symbols=active_entry_symbols(),max_candidates=FAST_RADAR_MAX_CANDIDATES,
            )
            _fast_ticker_snapshot=new_snapshot
            if not picks:
                _fast_radar_state.update({
                    "status":"ok","checked":0,"armed":0,"triggered":0,"last_error":"",
                    "last_run":started,"last_symbols":[],
                })
                return
            ...
            async def one(pick):
                try:
                    async with sem:
                        analyzer=(_fast_analyze_short_symbol if pick.source=="ticker_impulse" else _fast_analyze_main_symbol)
                        signal=await asyncio.wait_for(analyzer(pick.symbol),timeout=75)
                    if not signal:
                        return (pick.symbol,"REJECT")
                    if core.was_sent_recently(
                        signal.symbol,signal.side,core.SIGNAL_COOLDOWN_HOURS,signal.timeframe
                    ):
                        return (pick.symbol,"COOLDOWN")
                    signal,assessment,streak=await _arm_and_check(signal,"auto_fast_radar")
                    if assessment.state=="CANCEL":
                        return (pick.symbol,"CANCEL")
                    if assessment.state=="READY" and streak>=2:
                        outcome=await _trigger_arm(context,signal.entry_now_arm_id,assessment)
                        if outcome and outcome[0]=="PRODUCTION":
                            return (pick.symbol,"TRIGGERED")
                    return (pick.symbol,"ARMED")
                except Exception as exc:
                    core.log.warning(
                        "V11.13 Fast Radar failed %s: %s: %s",pick.symbol,type(exc).__name__,exc
                    )
                    return (pick.symbol,"ERROR")

            outcomes=await asyncio.gather(*(one(p) for p in picks))
            armed=sum(1 for _,state in outcomes if state in {"ARMED","TRIGGERED"})
            triggered=sum(1 for _,state in outcomes if state=="TRIGGERED")
            _fast_radar_state.update({
                "status":"ok","checked":len(picks),"armed":armed,"triggered":triggered,
                "last_error":"","last_run":started,"last_symbols":[p.symbol for p in picks],
            })
            core.log.info(
                "V11.13 Fast Radar checked=%s armed=%s triggered=%s symbols=%s",
                len(picks),armed,triggered,",".join(p.symbol for p in picks)
            )
        except Exception as exc:
            _fast_radar_state.update({
                "status":"error","checked":0,"armed":0,"triggered":0,
                "last_error":f"{type(exc).__name__}: {exc}","last_run":started,
            })
            core.log.exception("V11.13 Fast Radar cycle failed")
...
async def news_status_v114(update,context):
    ...
    intel=dict(snapshot.get("global_intel") or {})
    items=list(snapshot.get("items") or [])[:6]
    lines=[
        "🌐 <b>YK GLOBAL NEWS INTELLIGENCE</b>","━━━━━━━━━━━━━━━━━━",
        f"Источники: <b>{int(snapshot.get('sources',0))}/{int(snapshot.get('source_total',0))}</b> · global-extra <b>{int(intel.get('extra_sources_ok',0))}/{int(intel.get('extra_sources_total',0))}</b>",
        f"High-impact 24ч: <b>{int(snapshot.get('high_impact_count',0))}</b> · breaking: <b>{len(snapshot.get('breaking_events') or [])}</b>",
        "⚠️ Новость запускает перепроверку рынка, но сама по себе не разрешает вход.",
    ]
    for i,event in enumerate(items,1):
        title=escape(str(event.get("title") or ""))[:280]
        source=escape(str(event.get("source") or "?"))
        cat=escape(str(event.get("category") or "GENERAL"))
        bias=escape(str(event.get("trade_bias") or "AMBIGUOUS"))
        conf=float(event.get("confidence") or 0)
        lines += ["",f"<b>{i}. {title}</b>",f"{source} · {cat} · {bias} · confidence {conf*100:.0f}%"]
    return await update.effective_message.reply_text(
        "\n".join(lines),parse_mode=ParseMode.HTML,disable_web_page_preview=True,reply_markup=main_menu()
    )


def breaking_news_payload_v11130(event):
    return news_alert_text(event)


core._breaking_news_payload=breaking_news_payload_v11130


async def _news_rescan_runner(context):
    global _news_rescan_task
    try:
        if _original_news_triggered_scans is not None:
            await _original_news_triggered_scans(context)
    except Exception:
        core.log.exception("V11.13 background news-triggered re-scan failed")
    finally:
        _news_rescan_task=None


async def schedule_news_triggered_scans_v11130(context):
    """Release the news watcher immediately; keep at most one heavy re-scan running."""
    global _news_rescan_task
    if _original_news_triggered_scans is None:
        return
    if _news_rescan_task is not None and not _news_rescan_task.done():
        core.log.info("V11.13 news re-scan coalesced: prior re-scan still running")
        return
    application=getattr(context,"application",None)
    if application is not None and hasattr(application,"create_task"):
        _news_rescan_task=application.create_task(
            _news_rescan_runner(context),name="v11130-news-market-rescan"
        )
    else:
        _news_rescan_task=asyncio.create_task(
            _news_rescan_runner(context),name="v11130-news-market-rescan"
        )


if _original_news_triggered_scans is not None:
    core._run_news_triggered_scans=schedule_news_triggered_scans_v11130
...
def early_watch_text():
    state=dict(_fast_radar_state or {})
    symbols=[str(x) for x in (state.get("last_symbols") or []) if x]
    arms=active_entry_rows(10)
    breaking=list(news_breaking_symbols())
    lines=[
        "👀 <b>EARLY WATCH · V11.18.0</b>",
        "━━━━━━━━━━━━━━━━━━",
        f"Fast Radar: <b>{escape(str(state.get('status','warming')).upper())}</b> · interval <b>{FAST_RADAR_INTERVAL_SEC}s</b>",
        f"Последний цикл: checked <b>{int(state.get('checked',0) or 0)}</b> · armed <b>{int(state.get('armed',0) or 0)}</b> · triggered <b>{int(state.get('triggered',0) or 0)}</b>",
    ]
    if symbols:
        lines.append("Radar focus: <b>"+escape(", ".join(symbols[:8]))+"</b>")
    if breaking:
        lines.append("News focus: <b>"+escape(", ".join(str(x) for x in breaking[:8]))+"</b>")
    if arms:
        lines += ["","<b>ARMED / наблюдение:</b>"]
        for row in arms[:6]:
            lines.append(
                f"• {escape(str(row.get('symbol','?')))} · {escape(str(row.get('side','?')))} · "
                f"{escape(str(row.get('timeframe','?')))} · confirm <b>{int(row.get('confirm_streak',0) or 0)}</b> "
                "· required <b>2 / protected 3</b>"
            )
    else:
        lines += ["","Сейчас нет ARMED-кандидатов."]
    lines += [
        "",
        "<i>EARLY WATCH не является командой на вход. Он только показывает кандидатов, которые бот перепроверяет до полноценного ENTRY NOW.</i>"
    ]
    return "\n".join(lines)
...
async def callback_v112(update,context):
    ...
    # Keep Telegram buttons responsive: acknowledge immediately and detach only
    # heavy read/scan actions. Trading concurrency is still serialized by the
    # existing scan/entry locks inside those handlers.
    heavy={
        "scan":core.scan_cmd,"short_scan":core.short_scan_cmd,
        "news":core.news_status,"system":core.system_status,
        "movers":core.movers,"prices":core.prices,
    }
    if data in heavy:
        await query.answer("Запущено")
        _spawn_ui_task(heavy[data](update,context),data)
        return None
    ...
    if data=="v115:spot":
        await query.answer("Spot scan запущен")
        _spawn_ui_task(spot_cmd(update,context),"spot")
        return None
    ...
    if data=="v1114:performance":
        await query.answer("Готовлю отчёт")
        _spawn_sync_text_reply(query,performance_report_text,"performance")
        return None
    if data=="v1114:early":
        await query.answer()
        return await query.message.reply_text(early_watch_text(),parse_mode=ParseMode.HTML,reply_markup=main_menu())
    if data=="v1116:adaptive":
        await query.answer("Готовлю отчёт")
        _spawn_sync_text_reply(query,adaptive_edge_report_text,"adaptive")
        return None
    if data=="v1116:replay":
        await query.answer("Готовлю replay")
        _spawn_sync_text_reply(query,replay_recent_text,"replay")
        return None
    if data=="v1117:risk":
        await query.answer()
        r=_last_final_risk
        text=("🧿 <b>FINAL RISK GATE · V11.18.0</b>\n━━━━━━━━━━━━━━━━━━\n"
              f"Status: <b>{escape(str(r.get('status','WARMING')))}</b>\n"
              f"Symbol: <b>{escape(str(r.get('symbol','—') or '—'))}</b>\n"
              f"Score: <b>{float(r.get('score',0) or 0):.0f}/100</b>\n"
              f"Reason: {escape(str(r.get('reason','—')))}")
        return await query.message.reply_text(text,parse_mode=ParseMode.HTML,reply_markup=main_menu())
    if data=="v1117:challenger":
        await query.answer("Готовлю отчёт")
        _spawn_sync_text_reply(query,challenger_report_text,"challenger")
        return None
    if data=="v1117:attribution":
        await query.answer("Готовлю отчёт")
        _spawn_sync_text_reply(query,attribution_report_text,"attribution")
        return None
    if data=="v118:edgelab":
        await query.answer("Готовлю отчёт")
        _spawn_sync_text_reply(query,v1180_lab_text,"edgelab")
        return None
    if data=="v118:manager":
        await query.answer("Готовлю отчёт")
        _spawn_sync_text_reply(query,v1180_manager_text,"manager")
        return None
    ...
    if data=="v116:spotwatch":
        await query.answer("Готовлю отчёт")
        _spawn_sync_text_reply(query,spot_watch_text,"spotwatch")
        return None
    if data=="v1142:entrynow":
        await query.answer("Готовлю статус")
        _spawn_sync_text_reply(query,entry_now_status_text,"entrynow-status")
        return None
    if data=="v1142:active":
        await query.answer("Готовлю статус")
        _spawn_sync_text_reply(query,futures_active_text,"active-status")
        return None
    ...
    if data=="v112:health":
        ...
        _spawn_ui_task(_health_reply(),"health")
        return None
    if data=="v11:menu":
        await query.answer()
        return await query.message.reply_text("◈ <b>YK CONTROL CENTER · V11.18.0</b>\n<i>Выбери режим или аналитику.</i>",parse_mode=ParseMode.HTML,reply_markup=main_menu())
...
async def system_v112(update,context):
    ...
    try:
        ...
        text=(
            "🛡 <b>KORKOVTS V11.18.0 · REGIME + PORTFOLIO DEFENSE</b>\n"
            "━━━━━━━━━━━━━━━━━━\n"
            ...
            f"{system_extra()}\n"
            f"🚨 Entry monitor ARMED <b>{len(arms)}</b> · READY/persisting <b>{ready_arms}</b>\n"
...
async def lease_heartbeat_job(context=None):
    ok=await asyncio.to_thread(heartbeat_v11110_lease)
    if not ok:
        core.log.critical("V11.12 singleton lease lost; stopping application")
        app=getattr(context,"application",None) if context is not None else None
        if app is not None and hasattr(app,"stop_running"):
            app.stop_running()
        raise RuntimeError("V11.12 singleton lease lost")


async def alerts_on_v11130(update,context):
    core.subscribe(update.effective_chat.id,True)
    await update.effective_message.reply_text(
        "✅ <b>AUTO INTELLIGENCE ВКЛЮЧЁН</b>\n"
        "📡 Полный Futures-скан и статус: <b>каждые 10 минут</b>\n"
        "⚡ Fast Radar: <b>каждые 60 секунд</b>\n"
        "🚨 ENTRY NOW: проверка <b>каждые 30 секунд</b>, отправка при 2/2 или 3/3 по режиму\n"
        "🌐 Global/Crypto Shock Radar: <b>каждые 60 секунд</b>\n\n"
        "Если входов нет, 10-минутный статус всё равно придёт. "
        "Новость не является входом: сделка публикуется только после полного market confirmation.",
        parse_mode=ParseMode.HTML,reply_markup=main_menu()
    )

core.alerts_on=alerts_on_v11130
...
async def post_init_v112(application):
    ...
    _futures_l2_task=asyncio.create_task(futures_l2_monitor(),name="v11110-futures-local-orderbook")
    application.job_queue.run_repeating(
        lease_heartbeat_job,interval=30,first=15,name="v11110-singleton-lease"
    )
    application.job_queue.run_repeating(
        entry_now_monitor_job,interval=30,first=25,name="v1142-entry-now-monitor"
    )
    application.job_queue.run_repeating(
        fast_radar_job,interval=FAST_RADAR_INTERVAL_SEC,first=70,name="v11130-fast-radar"
    )
    application.job_queue.run_repeating(
...
async def post_shutdown_v112(application):
    global _live_task,_spot_book_task,_futures_l2_task
    # V11.17.2: detached Telegram jobs must not survive shutdown and hold DB/network resources.
    pending=list(_ui_background_tasks)
    for task in pending:
        task.cancel()
    if pending:
        await asyncio.gather(*pending,return_exceptions=True)
    _ui_background_tasks.clear()
    if _live_task:
        _live_task.cancel()
    ...
    if _spot_book_task:
        ...
        _spot_book_task=None
    if _futures_l2_task:
        await stop_futures_l2_monitor()
        _futures_l2_task.cancel()
        try: await _futures_l2_task
        except asyncio.CancelledError: pass
        _futures_l2_task=None
    ...
    try:
        ...
        await asyncio.to_thread(release_v11110_lease)
