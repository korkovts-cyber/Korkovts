"""V11.21.3 · Signal Engine Rebalance + fresh-cohort isolation."""
...
FUTURES_COHORT="11.21.3-signal-engine"
SPOT_COHORT="11.21.3-spot-signal-engine"
...
def install(base):
    ...
    # under V11.21.3 rules, and keep forward statistics in a clean cohort.
    ...
    def strong_annotate(signal):
        ...
        signal.feature_snapshot.setdefault("strong_consensus_v11150",{}).update({
            "schema":"11.21.3-strong-consensus-compat",
    ...
    # generated by the exact same V11.21.3 assessment.
    base.annotate_strong_signal=strong_annotate
    base.annotate_strong_signals=strong_annotate_many

    # Fresh-cohort isolation. V11.21 materially changed geometry and selection.
    # Old learned/calibration layers remain observable but cannot hard-reject
    # this new cohort. Live market, execution, L2, flow, ADL, crowding, news and
    # stop/TP invalidation remain authoritative.
    original_meta_decide=base.meta_decide
    def meta_decide_shadow(signal):
        signal,decision=original_meta_decide(signal)
        try:
            signal.feature_snapshot.setdefault("v11212_cohort_isolation",{}).update({
                "meta_original_ready":bool(getattr(decision,"ready",False)),
                "meta_original_eligible":bool(getattr(decision,"eligible",True)),
                "meta_shadow_only":True,
            })
        except Exception:
            pass
        if bool(getattr(decision,"ready",False)) and not bool(getattr(decision,"eligible",True)):
            try:
                decision=replace(decision,ready=False,eligible=True)
            except Exception:
                try:
                    ...
                    data.update({"ready":False,"eligible":True})
                    decision=SimpleNamespace(**data)
        return signal,decision
    base.meta_decide=meta_decide_shadow

    original_adaptive_gate=base._adaptive_gate
    def adaptive_gate_shadow(signal):
        ok,assessment=original_adaptive_gate(signal)
        if not ok:
            try:
                signal.feature_snapshot.setdefault("v11212_cohort_isolation",{}).update({
                    "adaptive_original_eligible":False,
                    "adaptive_reason":str(getattr(assessment,"reason","")),
                    "adaptive_shadow_only":True,
                })
            except Exception:
                pass
            return True,assessment
        return ok,assessment
    base._adaptive_gate=adaptive_gate_shadow

    original_entry_negative_penalty=base.entry_negative_penalty
    def entry_negative_penalty_shadow(signal,*args,**kwargs):
        penalty,stats=original_entry_negative_penalty(signal,*args,**kwargs)
        stats=dict(stats or {})
        stats.update({
            "v11212_original_penalty":float(penalty or 0),
            "v11212_shadow_only":True,
            "v11212_reason":"fresh strategy cohort; old entry history cannot reject",
        })
        return 0.0,stats
    base.entry_negative_penalty=entry_negative_penalty_shadow

    def spot_was_sent_recently_current(symbol,hours=72):
        # Deduplicate inside the current strategy cohort only. An older engine's
        # delivery must not suppress the first valid V11.21.3 BUY for 72 hours.
        spot_db.init()
